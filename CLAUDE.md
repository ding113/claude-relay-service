...
对于 v2,先行不开发Claude OAuth 模块.v2 的功能与 v1 有以下不同:
  初版只支持 console API（自定义端点）的 claude 和 codex,不提供oauth
  方式,降低复杂度.v2
  需要实现多上游\相同优先级时的会话粘性和自动重试.不同于 v1
  需要达到禁用阈值才禁用通道,v2
  的核心逻辑是,不管缓存成本,只要一个渠道报错,当前请求应当无感交由另一渠
  道进行重试,直到获取到响应,重试期间,用户侧应当完全无感. v2 相比
  v1,应该更加注重代码可复用性\简洁性,使用标准的分级日志,不在日志和代码
  的任何位置使用 emoji,确保代码的专业性和可维护性. v2
  首先应当实现的功能是,对数据结构进行梳理,确定数据库内是否有需要修改的
  字段,如果不需要修改,应当全量兼容 v1 数据库.
  在此之后,应当从最基本的用户登录（依赖 env 指定的凭据或初始化自动生成,
  不允许注册,只有管理员可登录,因此无需用户管理,用户直接凭借自己的 api
  key 即可查询,这一逻辑和 v1 保持一致） 
