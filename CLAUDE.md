...
│   │   │   │   ├── health/     # 健康检查
│   │   │   │   ├── auth/       # 认证模块（待开发）
│   │   │   │   ├── apikey/     # API Key 管理（待开发）
│   │   │   │   ├── account/    # 账户管理（待开发）
│   │   │   │   ├── relay/      # 转发核心（待开发）
│   │   │   │   └── stats/      # 统计查询（待开发）
│   │   │   └── server.ts       # Fastify 服务器入口
│   │   ├── tests/              # 单元测试
│   │   │   └── core/redis/     # 130 个测试（✅ 全部通过）
│   │   ├── .env.example        # 环境变量示例（兼容 v1）
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   └── vitest.config.ts
│   │
...
│
├── scripts/                    # 工具脚本
│   └── migrate-data.ts         # 数据迁移脚本（待开发）
│
├── docker/
│   ├── Dockerfile.v2           # v2 Docker 镜像（待更新）
│   └── docker-compose.v2.yml   # v2 容器编排（待更新）
│
├── CLAUDE.md                   # 本文档
├── V2_REFACTORING_PLAN.md      # 原始重构计划（参考）
...
### Phase 2: 认证与管理员登录（进行中）

**目标**: 实现管理员登录，支持 JWT 认证

**任务清单**:
- [ ] 实现 JWT 中间件（基于 @fastify/jwt）
- [ ] 管理员凭据管理
  - [ ] 环境变量配置（ADMIN_USERNAME/ADMIN_PASSWORD）
  - [ ] 自动生成管理员账号（首次启动）
  - [ ] BCrypt 密码哈希
- [ ] 登录 API
...
### Phase 3: API Key 管理模块

**目标**: 完整的 API Key CRUD + 使用统计查询

**任务清单**:
- [ ] API Key Service 业务逻辑
  - [ ] 创建 API Key（生成、哈希、存储）
  - [ ] 查询 API Key（列表、详情、统计）
  - [ ] 更新 API Key（配置、状态）
  - [ ] 删除 API Key（软删除 + 恢复）
